...
def main(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_sampler=RASampler(len(train_dataset), args.batch_size, 1, args.ra, shuffle=True, drop_last=True), collate_fn=collate_fn)
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=args.workers, collate_fn=collate_fn)
...
def train(train_loader, model, criterion, optimizer, epoch, scheduler,  args):
    ...
    for i, (images, target) in enumerate(train_loader):
        if (not args.no_cuda) and torch.cuda.is_available():
            # print(f'type of images: {type(images)}, type of target: {type(target)}')
            # print(f"images's elements: {images[0]}")
            images = [img.cuda(args.gpu, non_blocking=True) for img in images]
            # target['image_id'] = label
            target = target['image_id'].cuda(args.gpu, non_blocking=True)

            # target['bbox']
            # print(f"target's data: {target}")
            # print(f"target's elements: {target[0]}")
            # target_boxes = [t['boxes'].cuda(args.gpu) for t in target]
            # target = [t['image_id'].cuda(args.gpu) for t in target]
            # print(f"target's elements: {target[0]}")
