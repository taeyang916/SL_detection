...
def collater(images, annotations):
    ...
    for i in range(batch_size):
        ...
        for j in range(3):
            padded_imgs[i, :int(img.shape[0]), :int(img.shape[1]), j] = tensor_img
    ...
    if max_num_annots > 0:
        ...
        if max_num_annots > 0:
            for idx, annot in enumerate(annots):
                #print(annot.shape)
                tensor_annot = torch.Tensor(annot)
                if annot.shape[0] > 0:
                    annot_padded[idx, :annot.shape[0], :] = tensor_annot
    else:
